...
momdptf = PartialToMixedObs((:pos,),(:rocks,))
momdp = transform(momdptf,testrs.model)
#Test Sampling/Rand
momdp[:xp](;x=rand(support(momdp[:xp])),a=rand(support(momdp[:a])),y=rand(support(momdp[:yp])))
momdp[:yp](;x=([1,1],),a=2,y=rand(support(momdp[:yp])),xp=([1,2],))
...
pdf(momdp[:yp],sp;x=([1, 1],), y= (Bool[0, 0, 0],),a=2,xp=([1, 2],))
pdf(momdp[:yp],([0,0,1],);x=([1, 1],), y= (Bool[0, 0, 0],),a=2,xp=([1, 2],))
